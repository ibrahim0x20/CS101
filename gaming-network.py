...
def create_data_structure(string_input):
    ...
    while i < len(string_input)-1:
        start = string_input[i].find(' ')
        idxTo = string_input[i].find('to')
        
        user = string_input[i][:start]
        friends = string_input[i][idxTo+3:]
        friends = friends.replace(' ', '')
        friends = friends.split(',')
        
        i = i + 1
        idxPlay = string_input[i].find('play')
        games = string_input[i][idxPlay+5:]
        games = games.split(',')
        ...
        network.update({user:{'friends':friends, 'games':games}})
        i = i + 1
...
def add_new_user(network, user, games):
    if user in network:
        return network
    network.update({user:{'friends':[], 'games':games}})
    return network
...
def get_secondary_connections(network, user):
    if user in network:
        ...
        for friend in friends:
            secondary = secondary + network[friend]['friends']
...
def count_common_connections(network, user_A, user_B):
    if user_A in network and user_B in network:
        count = 0
        for friend in network[user_A]['friends']:
...
print find_path_to_friend(net, "John", "Walter")
...
#>>>['Olive', 'Ollie', 'Freda', 'Mercedes', 'Walter', 'Levi', 'Jennie', 'Robin', 'John',
#'Bryant']
